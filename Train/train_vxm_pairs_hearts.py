from FYP_package.voxelmorph.voxelmorph_timeseries.models.networks import VxmDense
from FYP_package.voxelmorph.voxelmorph_timeseries.models.losses import Combined_NCC, Combined_MSE, BendingEnergy, Grad
...
def save_and_log_images(fixed, flo, epoch, batch_size, save_name="train_figure"):
    ...
    fig, axes = plt.subplots(num_imgs, 3, figsize=(10, 3*num_imgs))
...
def main(args):
    ...
    train_dataset = HeartDataset(args.data_path, frames=False, frame_gap=5)
    ...
    if args.use_NCC_loss == "True":
        similarity_loss = Combined_NCC()
        print("Using NCC sim Loss")
    else:
        similarity_loss = Combined_MSE()
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        for batch_idx, frames in enumerate(train_loader):   
            ...
            frames = torch.concat(frames, dim=1).float().to(device, non_blocking=True) # TODO need to convert this to float in the dataset
            t01 = time.perf_counter()

            # Forward pass
            moved_images, displacement_field = model(frames)
            t02 = time.perf_counter()

            # Compute loss (NCC + smoothness)
            smooth_loss = smoothness_loss.loss(displacement_field)
            sim_loss = similarity_loss.loss(frames, moved_images)
            ...
            if batch_idx % 500 == 0:  
                if args.use_wandb == "True":
                    save_and_log_images(frames, displacement_field, epoch, args.batch_size, save_name="mid_epoch_fig")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", type=str, default="D:/cardiac_dataset/reformattedreformatted/train", help="Path to the dataset")
